...
export const fetchLists = createAsyncThunk('lists/fetchLists', async () => {
  const response = await fetch(`http://localhost:5000/lists`); // Adjust endpoint as needed
  return response.json();
});

export const createList = createAsyncThunk('lists/addList', async (list) => {
...
export const listSlice = createSlice({
  name: 'lists',
  initialState: { lists: [], status: 'idle', error: null },
  reducers: {},
  extraReducers: (builder) => {
    builder
      .addCase(fetchLists.pending, (state) => {
